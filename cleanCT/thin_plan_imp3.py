...
def is_e_open(p, old_mat):
    n = get_neighbors(p, old_mat)
    # e-open if exactly two s-neighbors are white and they are not opposite
    s_points = get_s_points(p, old_mat)
    pairs = [('T','B'), ('N','S'), ('W','E')]
    white_keys = [k for k, v in s_points.items() if v == 0]
    if len(white_keys) != 2: return False
    for p1, p2 in pairs:
        if p1 in white_keys and p2 in white_keys: return False
    return True

def is_v_open(p, old_mat):
    # v-open if exactly three s-neighbors are white and no two are opposite
    s_points = get_s_points(p, old_mat)
    white_keys = [k for k, v in s_points.items() if v == 0]
    if len(white_keys) != 3: return False
    pairs = [('T','B'), ('N','S'), ('W','E')]
    for p1, p2 in pairs:
        if p1 in white_keys and p2 in white_keys: return False
    return True
...
def is_shape_point(p, old_mat):
    # condition 2: stricter surface-like shape point detection
    s = get_s_points(p, old_mat)
    for a, d in [('T','B'), ('N','S'), ('W','E')]:
        if s[a] == 0:
            m_plane = get_middle_plane(p, a, old_mat).copy()
            m_plane[1, 1] = 0
            # must be an interior point of a surface, not an edge/corner
            _, n_comp = label(m_plane, structure=np.ones((3,3)))
            if n_comp == 1 and np.sum(m_plane) >= 3:
                return True
    return False
...
def thin_3d_saha(voxels, final_thinning=True):
    ...
    while True:
        ...
        # 2nd: e-open logic with Condition 3
        for p_idx in np.argwhere(img == 0):
            p = tuple(p_idx)
            if is_e_open(p, old_img_mask):
